...
class Encoder_ResNet(nn.Module):  
    ...
    def forward(self, x):
        x = self.conv1(x) # (w/2,h/2,64)
        x = self.bn1(x)
        f1 = self.relu(x) # (w/2,h/2,64)
        df1 = self.maxpool(f1) # (w/4,h/4,64)
        r1 = self.layer1(df1)  # (w/4,h/4,64)
        return r1, [f1, df1, r1]
...
class Decoder_ResNet(nn.Module):
    ...
    def forward(self, conv2):
        r2 = self.layer2(conv2)  # (w/8. h/8, 512)
        r3 = self.layer3(r2)     # (w/16, h/16, 1024)
        r4 = self.layer4(r3)     # (w/32, h/32, 2048)
        return r4, [r2, r3, r4] 
...
def create_Encoder_ResNet(backbone, last_stride, pretrained=False, **kwargs):
    """
    ...
    if backbone == 'resnet50':
        model = Encoder_ResNet(block=Bottleneck_IBN, layers=[3, 4, 6, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
    elif backbone == 'resnet101':
        model = Encoder_ResNet(block=Bottleneck_IBN, layers=[3, 4, 23, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
    elif backbone == 'resnet152':
        model = Encoder_ResNet(block=Bottleneck_IBN, layers=[3, 8, 36, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
...
def create_Decoder_ResNet(backbone, last_stride, pretrained=False, **kwargs):
    """
    ...
    # define model
    if backbone == 'resnet50':
        model = Decoder_ResNet(block=Bottleneck_IBN, layers=[3, 4, 6, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
    elif backbone == 'resnet101':
        model = Decoder_ResNet(block=Bottleneck_IBN, layers=[3, 4, 23, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
    elif backbone == 'resnet152':
        model = Decoder_ResNet(block=Bottleneck_IBN, layers=[3, 8, 36, 3], ibn_cfg=('a', 'a', 'a', None), **kwargs)
