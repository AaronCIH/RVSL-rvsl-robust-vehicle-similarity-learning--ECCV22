...
## ref: https://github.com/Cihsaing/SJDL-Foggy-Vehicle-Re-Identification--AAAI2022/blob/master/SJDL/utils.py
## 1. Domain transformation plot
## 2. Rank10 result plot
################################################
# Domain transformation plot
def Render_plot(IMGS, save_full_path='./Result/'):
    """
    input image must to be the list of numpy datas
    arg:
        IMGS : set of image, Stage1: (inC, ganC, inF, ganF) or Stage2:(inp, ganC, ganF)
        inC : Syn clear input image
        inF : Syn foggy input image
        inp : Real clear / foggy input image
        ganC : Rendered clear image with foggy image
        ganF : Rendered foggy image with clear image
        save_full_path : save path with root/file.png
    """
    img_w, img_h, img_dims = IMGS[0].shape
    ...
    ####### process labels ########
    if len(IMGS) == 3:
        encoder = ["inC", "ganC", "inF", "ganF"]
    else:
        encoder = ["inp", "ganC", "ganF"]
