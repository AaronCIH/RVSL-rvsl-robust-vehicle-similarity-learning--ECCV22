#######################################################
# Stage 3. for real foggy training...
# Creator: Cihsiang
...
from models import build_Stage3_Model, convert_model 
from losses import Lstage3_reid_sup, Lstage3_reid_ea, Lstage3_gan_uns, Lstage3_gan_sup, Loss_discriminator, MSEloss
...
def train():
    ...
    logger = setup_logger('RVSL_stage3', output_dir, 0) # check
    logger.info('Using {} GPUS'.format(num_gpus))
    logger.info(args)
    logger.info('Running with config:\n{}'.format(cfg))
    logger.info("##############################################################")
    logger.info("# % TRAIN .....  Stage 3 !!!!!") # !!!!
    ...
    SYN_train_dl, REAL_train_dl, REAL_val_dl, num_query, SYN_num_classes, _ = make_FVRID_dataloader_REAL(cfg, datatype='foggy', num_gpus=num_gpus)

    # Step5. Build RVSL Stage3 Model    
    ####################################################################
    ...
    print("SYN_num_classes = %d" %(SYN_num_classes))
    model = build_Stage3_Model(cfg, SYN_num_classes)
    ...
    Lreid_sup = Lstage3_reid_sup(cfg, SYN_num_classes)
    Lreid_ea = Lstage3_reid_ea(cfg)
    Lgan_uns = Lstage3_gan_uns(cfg)
    Lgan_p2p = Lstage3_gan_sup(cfg)
    Ldisc, optimizer_D, lr_scheduler_D = Loss_discriminator(cfg)

    # Step7. Training
    print("#In[3]: Start traing--------------------------------")
    trainer = STAGE3_Trainer(cfg, model, SYN_train_dl, REAL_train_dl, REAL_val_dl, 
                             Lreid_sup, Lreid_ea, Lgan_uns, Lgan_p2p, 
...
class STAGE3_Trainer(object):
    def __init__(self, cfg, model, SYN_train_dl, REAL_train_dl, REAL_val_dl, 
        ...
        self.logger = logging.getLogger('RVSL_stage3.train') # !!! notice !!!
        ...
        if self.cfg.MODEL.TENSORBOARDX:
            ...
            self.writer = SummaryWriter(os.path.join(cfg.OUTPUT_DIR, 'Log_RVSL_Stage3'))
            self.writer_graph = SummaryWriter(os.path.join(cfg.OUTPUT_DIR, 'Log_RVSL_Stage3/model'))
    ...
    def step(self, SYN_batch, REAL_batch):
        ...
        # % Stage 3-2 REAL Foggy Forward
        ## % load data  % Model forward & gan & discriminator
        ####################################################################
        ## - real_train_dl:Getitem: {"imgs", "paths", "pids", "camids"}
        ## - Traing['real']: {'GlobalFeat_clear', 'GlobalFeat_foggy', 'GAN_clear', 'GAN_foggy'}
        #################################################################### 
        REAL_foggy, REAL_foggy_pth = REAL_batch['imgs'], REAL_batch['paths']
        REAL_foggy = REAL_foggy.cuda()
        REAL_Output = self.model([REAL_foggy], mode='real')
        ...
        Lgan_uns_real, Lgan_uns_list = self.Lgan_uns(REAL_foggy, REAL_gan_C)
        Lgan_p2p_real = self.Lgan_p2p(REAL_gan_F, REAL_foggy)
        Ldisc_real = self.Ldisc(REAL_gan_C, 'valid', vis)
        ### Weight Loss
        Wea_real, Wuns_real, Wp2p_real, Wdisc_real = 1, 0.01, 100, 0.5
        Lreal = Wea_real*Lreid_ea_real + Wuns_real*Lgan_uns_real + Wp2p_real*Lgan_p2p_real + Wdisc_real*Ldisc_real
        
        # % Stage 3-3 optimize + backward
        ####################################################################
        ...
        # % Stage 3-4 tune discriminator
        ####################################################################
        # optim_D : Disc(In_foggy, 'real'), Disc(Gf, 'fake')
        ####################################################################         
        # % FOR DISCRIMINATOR foggy
        self.optim_D.zero_grad()
        Real_loss = self.Ldisc(REAL_foggy, 'valid')
        temp = self.fack_buffer.push_and_pop(REAL_gan_F)
        ...
        SSIM_c_real, PSNR_c_real = eval_dh(REAL_gan_C, REAL_foggy)      
        SSIM_f_real, PSNR_f_real = eval_dh(REAL_gan_F, REAL_foggy)      

        # % record ....
        ####################################################################
        # ReID, GAN, SSIM, ACC
        # Syn_Plot, Real_Foggy_Plot
        ####################################################################
        if self.batch_cnt % self.cfg.SOLVER.LOG_PERIOD == 0:
            ...
            self.logger.info('-REAL: Lea:{:.5f}, Luns:{:.5f}, Lp2p:{:.5f}, Ldisc:{:.5f}'
                             .format(Lreid_ea_real.cpu().item(), Lgan_uns_real.cpu().item(), Lgan_p2p_real.cpu().item(), Ldisc_real.cpu().item()))
            self.logger.info('-REAL: Ldc:{:.5f}, Lundcp:{:.5f}, Ltv:{:.5f}'
                             .format(Lgan_uns_list[0].cpu().item(), Lgan_uns_list[1].cpu().item(), Lgan_uns_list[2].cpu().item()))                             
       ...
       # plot gan result
        if self.cfg.TEST.VIS and (self.batch_cnt == self.train_epoch):
            ...
            name = "Epoch_" + str(self.train_epoch) + "_" + REAL_foggy_pth[sample_idx].split("/")[-1]
            save_full_path = sample_result_path + name
            Sinc = REAL_foggy.cpu().numpy()[sample_idx].transpose((1, 2, 0))
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for ct, batch in enumerate(tqdm(self.REAL_val_dl, total=len(self.REAL_val_dl), leave=False)):
                ...
                in_foggy = in_foggy.cuda()

                Output = self.model([in_foggy], mode='real')  
                ...
                loss_c, loss_f = self.mse_loss(Gc, in_foggy), self.mse_loss(Gf, in_foggy)
                SSIM_c, PSNR_c = eval_dh(Gc, in_foggy)
                SSIM_f, PSNR_f = eval_dh(Gf, in_foggy)
                ...
                if ct == vis_idx:
                    if self.cfg.TEST.VIS:
                        ...
                        save_full_path = sample_result_path + name 
                        Sinc = in_foggy.cpu().numpy()[sample_idx].transpose((1, 2, 0))
